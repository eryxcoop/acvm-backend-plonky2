...
fn test_backend_cannot_provide_witness_value_bigger_than_u8_for_u8_range_check() {
    ...
    let min_not_allowed_witness_value_field =
        F::from_noncanonical_u64(min_not_allowed_witness_value.into());
...
fn test_range_check_with_witness_value(witness_value: F, max_num_bits: u32) {
    ...
    let black_box_range_8_opcode =
        circuit_factory::black_box_range_opcode(public_input_witness, max_num_bits);
    let circuit = circuit_factory::circuit_with_single_opcode(
        black_box_range_8_opcode,
        vec![public_input_witness],
    );
...
fn _assert_backend_supports_bitwise_operation(
    operation: fn(Witness, Witness, Witness, u32) -> Circuit,
    bit_size: u32,
    a: GoldilocksField,
    b: GoldilocksField,
    ...
    let circuit = operation(
        public_input_witness_0,
